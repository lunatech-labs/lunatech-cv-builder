...
`saveCv()` itself no-ops (returns `true` without hitting the network) when the CV already exists and `hasUnsavedChanges()` is false. This matters because those implicit-save callers fire on every click regardless of whether the user actually edited anything — without the guard, clicking Preview PDF on an untouched CV would still `PUT` and the backend's `update_any` unconditionally sets `updated_at = NOW()` on every write, making an unmodified CV look freshly edited in the CV lists / Overview ranking.
