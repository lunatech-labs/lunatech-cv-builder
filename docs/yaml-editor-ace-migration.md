...
Decision record for why the hand-rolled YAML editor was replaced with the [Ace](https://ace.c9.io/) editor library, why Ace was chosen over CodeMirror 6, and why the CDN load is hardened the way it is. Claims below are backed by measurements taken during the work (Playwright scripts driving a real browser, computed-style/API reads, not visual impressions), not assumptions, see the "How this was verified" note under each finding.
...
- **`js-yaml` and `marked`** (the two other CDN `<script>` tags in this file) have no SRI, unlike Ace's. `marked.parse(review.report_markdown)` is also written straight into `innerHTML` with no sanitization. Both predate this migration and are a different blast radius (review-rendering / YAML-parsing, not the editor), so fixing them belongs in its own PR rather than bundled into Ace work — tracked here so the CDN-hardening story above doesn't read as complete.
