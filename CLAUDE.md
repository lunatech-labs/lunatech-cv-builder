...
`saveCv()` itself no-ops (returns `true` without hitting the network) when the CV already exists and `hasUnsavedChanges()` is false. This matters because those implicit-save callers fire on every click regardless of whether the user actually edited anything — without the guard, clicking Preview PDF on an untouched CV would still `PUT`, making an unmodified CV look freshly edited in the CV lists / Overview ranking. There's a matching backstop server-side: `Db::update_any` (`src/db.rs`) only writes the row (and only then bumps `updated_at`) when `yaml`/`name`/`label` actually differ from what's stored, so a PUT with unchanged content — from this guard failing, or from any other caller of the API — is a true no-op rather than just skipping the timestamp.
